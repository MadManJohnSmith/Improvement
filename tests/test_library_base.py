"""Biblioteca materializada: contrato, contenido y cobertura de escenarios.

Valida library/library.json, las skills base en library/base/, los patrones
de catálogo en library/catalog/ y sus escenarios: coherencia bidireccional,
procedencia registrada, criterios de activación acoplados al contenido,
límites de entrada y saneamiento. Fail-closed: cualquier desajuste rompe
la suite.
...
def all_entries(library):
    """(kind, entry, content_dir) para cada entrada seleccionable."""
    entries = [("base", e, BASE_DIR / e["name"])
               for e in library["base_skills"]]
    entries += [("catalog", e, CATALOG_DIR / e["name"])
                for e in library["catalog_patterns"]]
    return entries
...
class LibraryDocumentTest(unittest.TestCase):
    ...
    def test_catalog_pattern_present(self):
        self.assertGreaterEqual(len(self.library["catalog_patterns"]), 1)
        names = [c["name"] for c in self.library["catalog_patterns"]]
        self.assertEqual(len(names), len(set(names)),
                         "duplicate catalog pattern names")

    def test_provenance_sources_are_registered(self):
        ids = registered_source_ids()
        for _kind, entry, _d in all_entries(self.library):
    ...
    def test_mvp_skills_not_shadowed(self):
        ...
        self.assertFalse(names & mvp,
                         "library entries must not shadow MVP construction skills")
...
class LibraryContentTest(unittest.TestCase):
    """Cada entrada tiene contenido único, saneado y dentro de presupuesto."""

    def setUp(self):
        self.library = load_library()
        self.entries = all_entries(self.library)

    def test_every_entry_has_content_directory(self):
        for kind, entry, d in self.entries:
            with self.subTest(entry=entry["name"]):
                self.assertTrue(d.is_dir(), f"{kind} content missing")
                skill_md = d / "SKILL.md"
    ...
    def test_catalog_activation_criteria_coupled_to_content(self):
        ...
        for _kind, entry, d in self.entries:
            if "activation_criteria" not in entry:
                continue
            with self.subTest(entry=entry["name"]):
                text = (d / "SKILL.md").read_text(encoding="utf-8")
                self.assertIn("## Activación", text)
                for criterion in entry["activation_criteria"]:
                    self.assertIn(criterion, text)


class LibraryScenarioCoverageTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for p in sorted(CATALOG_DIR.glob("*/scenarios/*.json")):
            self.on_disk[p.stem] = p
    ...
    def test_each_entry_has_positive_and_negative(self):
        for _kind, entry, _d in all_entries(self.library):
            with self.subTest(entry=entry["name"]):
                types = set()
                for scenario_id in entry["scenarios"]:

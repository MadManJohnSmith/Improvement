...
El catálogo especializado comienza con su primer patrón activado por evidencia: `fastapi-openapi-contract-check` (dominio `python-backend`), contenido en `library/catalog/` con cuatro escenarios. Se activa solo ante señales observables (fastapi/pydantic en dependencias, routers/BaseModel/OpenAPI observados, encargo explícito de contratos); comprobación de solo lectura sobre OpenAPI reproducible, sin llamadas a servicios reales ni inferencia de seguridad desde el esquema, con fuente `fastapi-framework` fijada por observación directa. Los criterios de activación declarados en el contrato deben aparecer verbatim en el contenido del patrón, y la regresión lo exige. El catálogo no recibe más patrones sin evidencia de dominio y aceptación específica.

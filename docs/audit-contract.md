...
Los requisitos de doble perspectiva, prevención de recurrencia, verificación humana, memoria de decisiones y cifras de fuentes vigentes se aplican en las skills/Host/revisión. Este validador no los autentica ni demuestra ejecución. Existen servicios separados `decisions.py` y `metrics.py`, pero `audit.py consolidate` no los exige, enlaza ni valida; la evidencia humana y su validación mecánica específica siguen fuera de este contrato. Cambiar un prompt no implementa enforcement.

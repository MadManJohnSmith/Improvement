# Entrada de usuario: incorporar, auditar o reparar

El mensaje identifica proyecto y objetivo; el procedimiento vive aquí para no exigir prompts largos. Lee `docs/usage.md` para el protocolo vigente. Antes de ejecutar, muestra el estado compacto de preparación; si falta una precondición material, conserva `RETAINED` y no conviertas pasos mecánicos en preguntas.
...
- «Audita todo/completamente»: carga nativamente `workflow-complete-auditor`; descubre, particiona y consolida todas las unidades autorizadas en una cola externa. No pide el siguiente elemento.
- «Audita/revisa una unidad»: carga nativamente `workflow-auditor` y sigue una unidad dirigida. Una auditoría no habilita reparación.
- «Repara esta tarea/reparación continua»: carga `workflow-continuous-repair`, consume la cola externa y continúa unidades autorizadas; confirma solo permisos materialmente faltantes. No toca producto vivo por defecto.

...
- Si falta carga nativa, conserva el encargo en el perfil externo y devuelve preparación pendiente; no inventes ejecución del modo. Una nueva sesión Standard en el workspace es bootstrap, no un prompt requerido en cada turno.
...
7. Si la misión autoriza preparar skills para DSH, usa `--prepare-skills` para copiarlas al workspace externo en `.agents/skills/`, primero en dry-run y después con `--init`. La copia recursiva es explícita, idéntica es idempotente; conflicto bloquea hasta autorizar `--update-skills` con respaldo externo según docs/usage.md. Compara revisión/cuerpos antes de cargar; no uses `~/.dsh`, `.agents` del usuario ni el producto. Abre una nueva sesión Standard con ese workspace como raíz, no su padre; no requiere overlay. Comprueba catálogo y carga nativa según docs/setup-dsh.md; la copia por sí sola no lo demuestra.

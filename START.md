...
7. Si la misión autoriza preparar skills para DSH, usa `--prepare-skills` para copiarlas al workspace externo en `.agents/skills/`, primero en dry-run y después con `--init`. La copia es explícita y sin sobrescritura; no uses `~/.dsh`, `.agents` del usuario ni el producto. Abre una nueva sesión Standard con ese workspace como raíz, no su padre; no requiere overlay. Comprueba catálogo y carga nativa según docs/setup-dsh.md; la copia por sí sola no lo demuestra.
